...
genlog_mle <- function(parameters, data, hessian = F, alpha = 0.05){
  ...
  op <- optim(par=parameters, fn = genlogis.loglikehood, x=data,
              lower = c(0.00001,0.00001,0.00001, -Inf), upper = c(Inf,Inf,Inf,Inf),
...
#' ## Using generic parameter starting values
#' datas <- rgenlog(10000, 1.5,2,2, 0)
#' genlog_mle2(c(.5,1.6, 1.5, 0),datas)
#' 
#' ## Select parameters starting values with genlog_slider
#' datas <- rgenlog(10000, 1.5,2,2, 0)
#' genlog_slider(datas, return_var = 'parameters') ## choose parameters
#' genlog_mle2(parameters,datas)
#' 
#' @usage 
#' genlog_mle2(parameters, data, hessian = F, alpha = 0.05)
#' 
#' @details 
#' Maximum likehood estimation of parameters for the distribution proposed in this package.\cr
#' This function is an application of \code{optim} as a particular case needed for this 
...
genlog_mle2 <- function(parameters, data, hessian = F, alpha = 0.05){
  
    fn <- function(p) genlogis.loglikehood(p, x = data)
    gr <- function(p) numDeriv::grad(fn, p)
    ui <- rbind(c(1, 0, 0, 0), c(0,1,0,0), c(0,0,1,0))
    ci <- c(0,0,0)    
    op <- constrOptim(theta = parameters, f = fn, grad = gr,
...
#' ## Using generic parameter starting values
#' datas <- rgenlog(10000, 1.5,2,2, 0)
#' genlog_mle3(c(.5,1.6, 1.5, 0),datas)
#' 
#' ## Select parameters starting values with genlog_slider
#' datas <- rgenlog(10000, 1.5,2,2, 0)
#' genlog_slider(datas, return_var = 'parameters') ## choose parameters
#' genlog_mle3(parameters,datas)
#' 
#' @usage 
#' genlog_mle3(parameters, data, hessian = F, alpha = 0.05)
#' 
#' @details 
#' Maximum likehood estimation of parameters for the distribution proposed in this package.\cr
#' This function is an application of \code{optim} as a particular case needed for this 
...
genlog_mle3 <- function(parameters, data, hessian = F, alpha = 0.05){
  
  fn <- function(p) genlogis.loglikehood(p, x = data)
  ui <- rbind(c(1, 0, 0, 0), c(0,1,0,0), c(0,0,1,0))
  ci <- c(0,0,0)    
  op <- constrOptim(theta = parameters, f = fn,
